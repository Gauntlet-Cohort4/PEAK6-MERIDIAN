...
/// Creates a new strike market with Yes/No mints and a USDC vault.
/// The Phoenix market address is passed in but created off-chain.
pub fn handler(
...
/// Accounts required for `create_strike_market`.
#[derive(Accounts)]
...
pub struct CreateStrikeMarket<'info> {
    ...
    /// The ticker this market belongs to.
    #[account(
    ...
    /// USDC vault for this market (PDA-owned ATA).
    #[account(
        init,
        payer = admin,
    ...
    /// Phoenix Legacy market address (created off-chain).
