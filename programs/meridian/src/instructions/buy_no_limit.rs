...
/// Composite instruction: mint a Yes/No pair, then post Yes as a limit sell
/// on Phoenix Legacy. User gets No tokens immediately; Yes tokens are offered
/// on the order book at the specified price.
///
/// Steps:
///   1. Transfer USDC from user to vault
///   2. Mint Yes + No tokens
///   3. CPI to Phoenix: post Yes as limit sell at `price_in_ticks`
pub fn handler(
    ...
    require!(price_in_ticks > 0, MeridianError::InvalidStrikePrice);
    ...
    // 2. Mint Yes tokens to PDA account (for Phoenix listing).
    token::mint_to(
    ...
    // 3. CPI to Phoenix: post Yes as limit sell.
...
/// Accounts required for `buy_no_limit`.
#[derive(Accounts)]
pub struct BuyNoLimit<'info> {
    /// The user placing the limit order.
    #[account(mut)]
    ...
    /// PDA-owned YES token account (for Phoenix listing).
    #[account(mut)]
    pub pda_yes_account: Account<'info, TokenAccount>,

    /// PDA-owned quote token account.
    #[account(mut)]
